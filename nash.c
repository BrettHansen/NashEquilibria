...
////	Check if creating strategies will cause integer overflow
////	Memory overflow still a major concern
//	if (stages >= s_for_overflow(m, n) || stages >= s_for_overflow(n, m)) {
//		printf("\nToo many stages to calculate!\n");
//		exit(2);
//	}
...
//		Test max in row for nashity
	...
	printf("Total SGP NE for %d stage%s: %d", s, s == 1 ? "" : "s", sgp_count);
